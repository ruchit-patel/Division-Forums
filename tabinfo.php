  ...
  $cnt=0;
    ...
    while($row = mysqli_fetch_assoc($result)) {
      if($row["Tables_in_division"]=="wepBns" || $row["Tables_in_division"]=="wepMain" || $row["Tables_in_division"]=="wepStats" || $row["Tables_in_division"]=="wepTal"){
        ...
        else if($row["Tables_in_division"]=="Account" || $row["Tables_in_division"]=="gearMain" || $row["Tables_in_division"]=="gearMods" || $row["Tables_in_division"]=="gearPerformMod" || $row["Tables_in_division"]=="gearSkillsMod" || $row["Tables_in_division"]=="gearTalents" || $row["Tables_in_division"]=="gear_set_tals" || $row["Tables_in_division"]=="wepMods" || $row["Tables_in_division"]=="Extra" || $row["Tables_in_division"]=="Major_Attr" ||  $row["Tables_in_division"]=="Minor_Attr" || $row["Tables_in_division"]=="News"      ){
          continue;
        }
        else{
